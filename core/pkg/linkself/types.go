...
type Client interface {
	...
	// including the identity file and all storage. This is intended for
	// development and testing only. After calling this method, a new identity
	// will be generated on the next Start().
	//
	// WARNING: This operation is irreversible. All data will be permanently lost.
	//
	// DangerouslyDeleteAllData はノードを停止し、IDファイルやストレージを含む
	// すべての永続データを削除します。開発・テスト用途のみを想定しています。
	// このメソッド呼び出し後、次回の Start() で新しいIDが生成されます。
	//
	// 警告: この操作は取り消せません。すべてのデータが完全に失われます。
	DangerouslyDeleteAllData(ctx context.Context) error

	// InjectTestMessage simulates receiving a message without a real P2P connection.
	// This is intended for development and UI testing only.
	//
	// Security: fromDID MUST be a test DID (did:test:...) generated by GenerateTestDID.
	// Passing a real DID (did:key:...) will return an error to prevent accidental
	// confusion between real and injected messages.
	//
	// InjectTestMessage は、実際のP2P接続なしにメッセージ受信をシミュレートします。
	// 開発およびUIテスト専用です。
	//
	// セキュリティ: fromDID は GenerateTestDID で生成したテストDID (did:test:...)
	// でなければなりません。実際のDID (did:key:...) を渡すとエラーになります。
	InjectTestMessage(ctx context.Context, fromDID string, payload []byte) error
