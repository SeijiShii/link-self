...
type client struct {
	...
	// Paths stored for DangerouslyDeleteAllData.
	identityPath string // path to identity.json
	dataRootDir  string // data root directory (empty if no SuiteID)
}
...
func (c *client) Start(ctx context.Context, config Config) (*NodeInfo, error) {
	...
	if config.SuiteID != "" {
		...
		c.dataRootDir = dataroot.DIDDir(root, identity.DID)
...
// DangerouslyDeleteAllData stops the node and deletes all persistent data.
// See Client.DangerouslyDeleteAllData for detailed documentation.
func (c *client) DangerouslyDeleteAllData(ctx context.Context) error {
	// Stop the node first to release file handles.
	...
	if c.identityPath != "" {
		if err := os.Remove(c.identityPath); err != nil && !os.IsNotExist(err) {
			firstErr = fmt.Errorf("remove identity file: %w", err)
		}
	}

	// Delete data root directory for this DID.
	if c.dataRootDir != "" {
		if err := os.RemoveAll(c.dataRootDir); err != nil && firstErr == nil {
			firstErr = fmt.Errorf("remove data directory: %w", err)
		}
	}
...
// InjectTestMessage simulates receiving a message for testing.
// Only accepts did:test: DIDs to prevent confusion with real messages.
func (c *client) InjectTestMessage(ctx context.Context, fromDID string, payload []byte) error {
	if c.node == nil {
		return fmt.Errorf("node not started")
	}
	if !IsTestDID(fromDID) {
		return fmt.Errorf("InjectTestMessage requires a test DID (did:test:...), got %q", fromDID)
	}
	c.node.DispatchMessage(fromDID, payload)
	return nil
}
...
// testDIDPrefix is the prefix used by GenerateTestDID to clearly mark dummy DIDs.
const testDIDPrefix = "did:test:"

// GenerateTestDID generates a random DID for testing/development purposes.
// The returned DID uses the "did:test:" method prefix, which is clearly
// distinguishable from real DIDs ("did:key:"). Test DIDs cannot be used
// for P2P operations (Connect, SendMessage) — they are intended only for
// UI testing and dummy data seeding.
func GenerateTestDID() (string, error) {
	b := make([]byte, 16)
	if _, err := rand.Read(b); err != nil {
		return "", fmt.Errorf("generate random bytes: %w", err)
	}
	return testDIDPrefix + hex.EncodeToString(b), nil
}

// IsTestDID returns true if the given DID was generated by GenerateTestDID.
// Use this to guard against accidentally treating test data as real data.
func IsTestDID(did string) bool {
	return strings.HasPrefix(did, testDIDPrefix)
}
