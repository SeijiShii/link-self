...
| `src/join.ts` | （Go 未実装・TS 先行） | ✅ 2a（純ロジック+codec） | 参加ハンドシェイクの受理ロジック（`JoinService.accept`: 署名/期限・発行者/受理者の admin 判定・nonce 単回使用・addMember・スナップショット返却）と JSON wire codec。live libp2p 配線（`/linkself/join/1.0.0`）は Slice 2b |
