...
**概要:** コアモジュールの Node API（グループ単位）を利用し、1台のPC上で複数プロセス（複数ターミナル）として複数ノードを動かすシンプルなサンプルチャットアプリの設計・実装方針。  
**参照:** [グループの概念](group-concept.md)、[Phase 1 設計](phase1-design.md)

---

## 結論

- **シンプルなサンプルチャットアプリ**: 実装可能。コアの Node API（`New`, `Start`, `SetOnMessage`, `SendToGroup`, `ConnectToGroup`）を利用する。1 対 1 は **2 人グループ**のケースとして扱う（[グループの概念](group-concept.md) 参照）。
...
- **Phase 1 設計**では `core/cmd/linkself/` は「CLI 用（未実装）」とされているため、サンプルチャット用のエントリポイントを **`core/cmd/linkself/`** に置く（1バイナリで「チャットノード」を起動する形）。
...
- コアのみで **サンプルチャットアプリ** は実装可能。送受信は **グループ単位**（SendToGroup / ConnectToGroup）とし、1対1は 2 人グループのケースとして扱う。
- **1台のPCで複数ノード** は、同じバイナリを複数プロセス（複数ターミナル）で起動し、bootstrap でつなぐ形で再現できる。
- 実装量は「1エントリポイント + 対話ループ + identity 永続化 + グループ管理 + 簡単なドキュメント」程度に抑えられる。
