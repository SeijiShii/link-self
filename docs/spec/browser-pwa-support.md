...
| M3: sqlite-wasm + MyDB | ✅ **完了 + 実ブラウザ検証済み**（2026-07-07） | `SqliteWasmDatabase`（**永続は専用 Worker で OPFS SAHPool VFS**、`:memory:` は main thread）+ `MyDB`（KV+SQL、SQL 書き込みの devicesync ミラー = Go と同一配線）。`ts/browser-e2e` の Playwright で **4/4 PASS**: crossOriginIsolated / WS 直結 auth+echo / **OPFS リロード跨ぎ永続** / Web Locks 多タブ |
| ブラウザ実機 E2E で判明した実ブラウザ限定の落とし穴 | ✅ 対処済み | ① ブラウザは loopback dial を既定拒否 → localhost 接続は libp2p `connectionGater.denyDialMultiaddr:()=>false`（本番は公開リレー宛なので不要）。② OPFS SAHPool の `createSyncAccessHandle` は Worker のみ → sqlite は Worker で動かす必須（main thread は不可）。③ sqlite の wasm/worker 資産は Vite の `server.fs.allow` に含める |
