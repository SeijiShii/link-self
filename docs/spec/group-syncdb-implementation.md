...
> **注意（2026-04）:** グループのオーナー概念はロール DAG に吸収予定。`Owners` フィールドは廃止し、管理操作の権限はスイートのロール定義で決める方針に移行する。詳細は [sync-db-plan.md §9.1](sync-db-plan.md) を参照。以下の実装記録はロール DAG 移行前の状態を反映している。
